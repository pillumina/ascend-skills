...
`ascend-inference-profiling` skill. Read this file first when extending
...
> `.agents/skills/ascend-inference-profiling/tests/` enforce a subset.
...
| `hardware_capabilities.yaml` | **Contract** (active) | `characterize.py` | device peak FLOPs for hardware identification |
| `interpreting-outputs.md` | **Reference** (agent-facing, lazily-loaded) | LLM agent after pipeline run | How to read outputs, interpret config signatures, use characterization data. |
| `vllm-ascend/` | **Reference** (agent-facing, versioned) | LLM agent | Three-layer vLLM-Ascend knowledge. See `vllm-ascend/README.md`. |
...
| hardware capabilities / shape parsing | `--from-stage characterize` |
